...
| `PathProfile` | `path`, `when`, `station`, `invert`, `crown`, `wse`, `velocity`, `flow`, `conduit_of`, `node_stations`, `total_length`, `energy`, `times`, per-face `shape`/`rise`/`span`; properties `depth`, `is_surcharged`, `surcharge_margin`, `energy_grade`, `critical_wse`, `station_from_end` | One continuous profile chained along a ConduitPath, station accumulated across conduits and bridged gaps. `station_from_end` is RAS Mapper's pipe-profile x-axis for a downstream-traced path. Place nodes with `node_stations` (true position, summed conduit lengths), never at a face: faces sit half a cell off the node and move with a geometry's face count (the removed `node_at` did that, and put Armour Rd's J314 at two stations) |
